...
function brickEOTWInputHatchData::onTick(%this, %obj) {
	%client = %obj.getGroup().client;
    if (isObject(%player = %obj.rayCastPlayer()) && getTrustLevel(%player.client, %obj) >= 2 && %obj.attemptPowerDraw(0))
	{
...
function brickEOTWOutputHatchData::onTick(%this, %obj) {
    %client = %obj.getGroup().client;
    if (isObject(%player = %obj.rayCastPlayer()) && getTrustLevel(%player.client, %obj) >= 2 && isObject(%matter = getMatterType(getField(%obj.matter["Input", 0], 0))) && %obj.attemptPowerDraw(0))
	{
